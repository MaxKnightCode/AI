...
namespace GraphProgram
{
    internal class Graph<T> where T : IEquatable<T>
    {
        ...
        public bool RemoveEdge(Vertex<T> a, Vertex<T> b)
        {
            ...
            {
                if (a.Neighbors[i].StartingPoint.Value.Equals(a.Value) && a.Neighbors[i].EndingPoint.Value.Equals(b.Value))
                {
        ...
        */
        ...
        public VertexWrapper<T> JoStar(PriorityQueue<VertexWrapper<T>, float> Fronteir, List<VertexWrapper<T>> FronteirsToAdd, List<Vertex<T>> Visited)
        {
            ...
            for (int i = 0; i < FronteirsToAdd.Count; i++)
            {
                if (Visited.Contains(FronteirsToAdd[i].Vertex) == false)
                {
                    //Fronteir.Enqueue(FronteirsToAdd[i], FronteirsToAdd[i].Cost + HeurisiticCalc(FronteirsToAdd[i], Manhattan));
                }
            }
        ...
        /*
        public unsafe float HeurisiticCalc(VertexWrapper<T> startingPoint, Func<Vertex<Point>, Vertex<Point>, float> HeuristicType)
        {
            if (typeof(T) == typeof(Point))
            {
                Vertex<Point> fml = new Vertex<Point>((Point)(startingPoint.Vertex.Value));
                return HeuristicType(startingPoint.Vertex, Goal);
            }
            else throw new NotImplementedException("Make this more generic");
        }



        public static float Manhattan(Vertex<Point> node, Vertex<Point> goal)
        {
...
//modern ai's purpose is to make ai act rationaly, GET YOU CLOSER TO GETTING TO YOUR GOAL

//ABove, this is uninformed search!!